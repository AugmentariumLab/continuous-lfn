...
def convert_rays_to_plucker(positions, directions, use_manual_cross=False):
    ...
    """
    cross_prod = torch.cross(positions, directions, dim=-
                             1) if not use_manual_cross else cross(positions, directions)
...
def compute_image_gradients(image: torch.Tensor):
    """Computes image x and y gradients using a sobel filter.

    Args:
      image: Image to compute gradients for as (B, H, W, C) tensor.

    Returns:
      Tensor of shape (B, H, W, 2) containing the x, y, grayscale gradients.
    """
    if image.shape[1] > 1:
      image = 0.212671 * image[..., 0] + 0.715160 * image[..., 1] + 0.072169 * image[..., 2]
    sobel_filter_x = torch.tensor(
        [[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=image.dtype, device=image.device)
    sobel_filter_y = sobel_filter_x.transpose(0, 1)
    sobel_filters = torch.stack((sobel_filter_x, sobel_filter_y), dim=0)
    image_gradients = F.conv2d(image[:, None, :, :], sobel_filters[:, None, :, :], padding=1)
    return image_gradients.permute((0, 2, 3, 1))

def solve_quadratic(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, use_minus=False):
    """Solution to quadratic formula."""
    return (-b + (1 - 2 * use_minus) * torch.sqrt(b * b - 4 * a * c)) / (2 * a)
