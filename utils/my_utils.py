...
def spherical_to_cubemap(theta, phi):
    ...
    u[top_indices] = -np.tan(phi[top_indices]) * np.sin(theta[top_indices] -
                                                        np.pi)
    v[top_indices] = np.tan(phi[top_indices]) * \
        np.cos(theta[top_indices] - np.pi)
    side[top_indices] = 0
...
def panobasic_im2sphere(fov, sphere_width, sphere_height, im_width, im_height,
    ...
    delta_x = delta_x.reshape(
        (sphere_height, sphere_width)) + (im_width + 1) / 2
    delta_y = delta_y.reshape(
        (sphere_height, sphere_width)) + (im_height + 1) / 2
...
def append_text_to_image(image, text, font_size=0.5, font_thickness=1,
                         font=cv2.FONT_HERSHEY_SIMPLEX,
                         font_color=(0, 0, 0)):
    """Appends text to an image.

    Args:
      image: numpy array of size HxWx3
      text: string to append
      font_size: font size

    Returns:
      numpy array of size HxWx3
    """
    height, width, channels = image.shape
    if channels != len(font_color):
        raise ValueError("Font color must have the same number of channels as "
                         "the image.")
    num_rows_to_concatenate = int(80 * font_size)
    empty_rows = np.zeros((num_rows_to_concatenate, width, channels),
                          dtype=image.dtype)
    image = np.concatenate((image, empty_rows), axis=0)
    font_scale = 2 * font_size
    (text_w, text_h), text_b = cv2.getTextSize(
        text, font, font_scale, font_thickness)
    org = (int(0.5 * width - 0.5 * text_w),
           int(height+num_rows_to_concatenate-0.5 * text_h))
