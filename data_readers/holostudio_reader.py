...
CAMERA_PARAMETER_FILENAMES = (
    ...
    "cameraParametersColmap0505.txt",
)
VIDEO_FRAMES_DIRS = ["DespilledFrames", "BGMV2OutputBK"]
...
class HolostudioDataset(Dataset):
    ...
    def __init__(self, basedir: str, factor: int = 8, recenter: bool = True, bd_factor: float = 0.75,
                 ...
                 cache_lowres: bool = False, saliency_dirname: str = "saliency",
                 renderposes_rotations: float = 2, renderposes_flip_up: bool = False):
        ...
        self.renderposes_rotations = renderposes_rotations
        self.renderposes_flip_up = renderposes_flip_up
        self.cache_lowres = cache_lowres
        self.saliency_dirname = saliency_dirname
        ...
        if self.dataset_type is None:
            ...
            if ret is not None:
                ...
                (self.bgmv_dir, self.len, self.num_frames, self.num_views, self.images_list, self.masks_list,
                 self.saliency_list, self.original_height, self.original_width) = ret
        if self.dataset_type is None:
        ...
        if self.using_processed_camera_parameters:
            ...
            if self.renderposes_flip_up:
                self.render_poses_rotation_mat = np.stack(
                    (pca.components_[1], pca.components_[0], up), axis=1)
            else:
                self.render_poses_rotation_mat = np.stack(
    ...
    def _parse_images(self):
        ...
        saliency_dir_names = [self.saliency_dirname]
    ...
    def _parse_video_frames(self):
        bgmv_dir = [os.path.join(self.basedir, x) for x in VIDEO_FRAMES_DIRS if
        ...
        if self.load_saliency:
            saliency_list = glob.glob(os.path.join(bgmv_dir, "*", self.saliency_dirname, "*.jpg"))
            saliency_list = sorted(
                x for x in saliency_list if frame_num_extractor(x) < self.max_frames)
            if not saliency_list:
                print("Saliency not found")
                exit(1)
            if self.dropout_poses and self.include_dropout_only:
                saliency_list = [x for x in saliency_list if extract_view_num(
                    x) in self.dropout_poses]
            elif self.dropout_poses:
                saliency_list = [x for x in saliency_list if extract_view_num(
                    x) not in self.dropout_poses]
            if self.load_every_nth_view > 1:
                saliency_list = [x for i, x in enumerate(
                    saliency_list) if i % self.load_every_nth_view == 0]
        ...
        return bgmv_dir, self_len, num_frames, num_views, images_list, masks_list, saliency_list, height, width
    ...
    def _getitem_videoframes(self, item):
        ...
        if self.saliency_list:
            saliency = plt.imread(os.path.join(
                self.saliency_list[item])).astype(np.float32) / 255
            if self.factor != 1:
                saliency = cv2.resize(saliency, (0, 0), fx=1 / self.factor, fy=1 / self.factor,
                                      interpolation=self.resize_interpolation)
            return_data["saliency"] = saliency
        if self.using_processed_camera_parameters:
    ...
    def get_render_poses(self, num_views: int = 30):
        if num_views not in self.render_poses:
            if self.using_processed_camera_parameters:
                ...
                rots = self.renderposes_rotations
                zrate = 0.0
                transforms = []
                intrinsics = []
                ...
                c2w_path = c2w
                N_views = num_views
                N_rots = self.renderposes_rotations
